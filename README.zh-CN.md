...
[v0.6.2 发布说明](docs/releases/v0.6.2.md) ·
...
<p align="center">
  <img src="docs/assets/v0.4.0-release-hero.svg" alt="StaticDynamic v0.4.0 PEER 行波地震输入" width="900">
</p>
...
<p align="center">
  <img src="docs/assets/v0.4.0-static-dynamic-workflow.svg" alt="StaticDynamic v0.4.0 流程图：边界识别、地应力平衡、PEER 波输入、行波到时差、等效荷载" width="900">
</p>

### v0.6.0 斜入射验证

`v0.6.0` 已在 Abaqus/CAE 中完成全流程验证：五层
`30 m x 20 m x 20 m` 土体、1 m C3D8R 网格、地应力反力提取、
粘弹性边界转换，以及 32 度斜入射行波地震输入。动力 ODB 中已确认包含
位移 `U`、速度 `V` 和加速度 `A` 场输出。

<table>
  <tr>
    <td width="33%" align="center">
      <img src="docs/assets/v0.6.0-30x20x20-5layer-mesh.png" alt="v0.6.0 五层 1 m 网格验证模型" width="100%"><br>
      <sub>1 m 五层土网格。</sub>
    </td>
    <td width="33%" align="center">
      <img src="docs/assets/v0.6.0-30x20x20-springdashpot-interactions.png" alt="v0.6.0 粘弹性边界特征" width="100%"><br>
      <sub>粘弹性边界特征。</sub>
    </td>
    <td width="33%" align="center">
      <img src="docs/assets/v0.6.0-30x20x20-oblique-loads.png" alt="v0.6.0 斜入射等效地震荷载" width="100%"><br>
      <sub>32 度斜入射地震荷载。</sub>
    </td>
  </tr>
</table>
...
重启 Abaqus/CAE 后，插件菜单中应出现 `StaticDynamic v0.6.2`。
...
`LayeredSite` 是 `v0.4.0` 的场地分析过渡模块：它复用黏弹性边界已经读取的土层刚度和密度，生成等效竖向 `Vs` 剖面，导出 `SeismicSiteProfile.csv`，并对不同深度的边界节点施加分层传播到时差。该模式目前只是 1D 运动学到时修正，还不包含等效线性放大、反演去卷积或完整自由场柱耦合。

## 后续路线

`v0.5.0` 已加入地震波输入工作台的第一步：

- `Wave Scale`：对单位换算后的加速度、速度和位移时程统一调幅。
