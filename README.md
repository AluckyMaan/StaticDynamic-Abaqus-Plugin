...
This project is under active development. The 3D five-face viscous-spring boundary workflow has been tested on a regular homogeneous soil model. Layered soil boundary grouping is implemented for common horizontal layered models by reading adjacent boundary element materials. Version `0.3.0` adds practical PEER earthquake-motion input through equivalent spring-dashpot boundary nodal forces.
...
Restart Abaqus/CAE. The plugin should appear as `StaticDynamic v0.3.0` in the plugin menu.
...
The next planned development target is `v0.4.0`, focused on more realistic
earthquake input:

- incident-wave direction controls beyond a uniform global vector
- spatial arrival-time delay along side and bottom boundaries
