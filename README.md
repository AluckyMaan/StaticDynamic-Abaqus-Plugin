...
Current release version: `0.6.2`

Latest stable release: `0.6.2`

[Release notes: v0.6.2](docs/releases/v0.6.2.md) ·
[GitHub release](https://github.com/AluckyMaan/StaticDynamic-Abaqus-Plugin/releases/tag/v0.6.2)
...
Restart Abaqus/CAE. The plugin should appear as `StaticDynamic v0.6.2` in the plugin menu.
...
Version `0.5.0` adds two preprocessing controls before the equivalent boundary
loads are generated:

- `Wave Scale`: multiplies the loaded acceleration, velocity, and displacement
  records after unit conversion. Use it for amplitude scaling; use `Incident
  Vector` for direction changes in most workflows.
- `Baseline Correction = RemoveMean`: subtracts the mean from each loaded
  record before scaling. The run report writes mean removed and peak values
  before/after preprocessing under `WavePreprocess`.
...
In `v0.5.0`, `LayeredSite` can also read `Site Profile CSV`. The file may use a
header such as:

```csv
verticalCoordinate,equivalentVs,cumulativeTravelTime,sampleCount
0,250,0,1
10,320,0.035,1
```

Only `verticalCoordinate` and `equivalentVs` are required. If cumulative travel
time is omitted, the plugin sorts the profile by coordinate and integrates
travel time from the lowest coordinate with trapezoidal slowness.

## Roadmap

Later milestones will continue the earthquake-input workbench:
